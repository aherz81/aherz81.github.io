...
define(function() {
	...
	HTMLBuilder.prototype.buildUsersTable = function(allUsers) {
		...
		thead.append('<tr><td rowspan="2">Name</td><td colspan="4">High score</td><td rowspan="2"></td></tr>');
		thead.append('<tr><td class="text-right">Score</td><td class="text-right">Time [s]</td><td class="text-right">Kills</td><td class="text-right">Cloaked Text</td></tr>');
		...
		for (var i = 0; i < allUsers.length; i++) {
			...
			row.append('<td class="text-right">' + (score ? Math.ceil(100.0*score.kills/score.time*(score.cloak? 1.5:1.0)) : '-') + '</td>');
			row.append('<td class="text-right">' + (score ? score.time : '-') + '</td>');
			row.append('<td class="text-right">' + (score ? score.kills : '-') + '</td>');
			row.append('<td class="text-right">' + (score ? score.cloak : '-') + '</td>');
