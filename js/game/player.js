...
define(function() {
	...
	var playerAnimationFrames = 20;

	/** The Player object. */
	function Player() {
		var img = new Image();
		img.src = 'img/game/player-sprite-new.png';
		var i;
		var imgWidth = 20*95;
		var imgHeight = 151;
		var spriteWidth = imgWidth/playerAnimationFrames;

		for(i=0;i<imgWidth/spriteWidth;i++)
		{
			playerAnimation.stand.push({ x: i*spriteWidth, y: 0, width: spriteWidth, height: imgHeight });
		}
...
/*
	...
	Player.prototype.shootat = function(x,y) {
		if (!self.paused && !self.disabled) {
			var angle = Math.atan((y - this.sprite.getY()) / (x - this.sprite.getX()));
			this.sprite.setRotation(angle);
			this.blood.setRotation(angle);
			if (x >= this.sprite.getX()) {
				this.sprite.rotateDeg(-90);
				this.blood.rotateDeg(-90);
			} else {
				this.sprite.rotateDeg(90);
				this.blood.rotateDeg(90);
			}
			if (this.shootCallback) {
				var points = [ this.sprite.getX(), this.sprite.getY(), x, y ];
				this.shootCallback(points);
			}
		}
	};	
