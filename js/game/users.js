...
define(function() {
	 ...
	 */
	Users.prototype.updateScore = function(time, kills, cloak) {
		var score = new Score(time, kills, cloak);
	...
	/** The Score object. */
	function Score(time, kills, cloak) {
		this.time = time;
		this.kills = kills;
		this.cloak = cloak;
	}
	...
	var scoreComparator = function(one, two) {
		...
		if (!one) return +1;
		if (!two) return -1;
		return two.kills/two.time*(two.cloak? 1.5:1.0)-one.kills/one.time*(one.cloak? 1.5:1.0);
	};
